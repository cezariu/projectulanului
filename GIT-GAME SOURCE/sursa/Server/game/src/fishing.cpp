...
#ifdef TEXTS_IMPROVEMENT
	ch->ChatPacketNew(CHAT_TYPE_INFO, 98, "%d#%d", fished.size(), total_count);
#endif
}
...
#ifdef TEXTS_IMPROVEMENT
				ch->ChatPacketNew(CHAT_TYPE_INFO, 248, "");
#endif
				break;
			case USED_SHELLFISH:
...
#endif
#ifdef TEXTS_IMPROVEMENT
	ch->ChatPacketNew(CHAT_TYPE_INFO, 116, "%s", item->GetName());
#endif
	item->SetCount(0);
...
namespace fishingnew
{
	...
	DWORD GetFishCatchedVnum(BYTE normal_chance, BYTE rare_chance, bool second) {
		if (number(1, 100) >= BYTE(normal_chance-rare_chance)) {
