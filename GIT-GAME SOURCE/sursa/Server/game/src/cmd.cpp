...
struct command_info cmd_info[] =
{
	...
	{ "map1",	do_map1,	0,						POS_DEAD, GM_IMPLEMENTOR },
...
// #if defined(ENABLE_ANTI_FLOOD)
	// if (ch && !ch->IsGM()) {
		// if (thecore_pulse() > ch->GetAntiFloodPulse(FLOOD_CMD) + PASSES_PER_SEC(1)) {
			// ch->SetAntiFloodCount(FLOOD_CMD, 0);
			// ch->SetAntiFloodPulse(FLOOD_CMD, thecore_pulse());
		// }

		// if (ch->IncreaseAntiFloodCount(FLOOD_CMD) >= 50) {
			// LPDESC d = ch->GetDesc();
			// if (d) {
