...
#	define INTRUSIVE_HANDSHAKE_LIMIT 10 /* (handshakes)
	...
	*/
#	define INTRUSIVE_HANDSHAKE_BAN TRUE /* (choice)
...
class DESC
{
	...
	public:
		...
		void 			SetLanguage(BYTE bLanguage) { m_accountTable.bLanguage = bLanguage; sys_log(0, "FINDME : language set to %u", bLanguage);}
...
#ifdef ENABLE_HWID
		const char * GetHwid() { return m_accountTable.hwid; };
#endif
