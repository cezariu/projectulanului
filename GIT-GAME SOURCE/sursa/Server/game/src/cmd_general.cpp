...
#if defined(ENABLE_BUGFIX_OFFLINESHOP_09042022)
	if (!ch->CanWarp()) {
		return;
	}
#endif
...
ACMD(do_in_game_mall)
{
	...
	char sas[33];
	MD5_CTX ctx;
	const char sas_key[] = "lkqOcF9KvqotbVHHNI+it9L1OtoAcE/GM5ha56QYRJY";
	...
	snprintf(buf, sizeof(buf), "mall https://rubria.to/ishop/?pid=%u&lang=%s&sas=%s",
...
ACMD(do_rune_charge)
{
	...
	if (pkBottle->GetCount() > 1) {
		...
		item2->SetSocket(0, pkBottle->GetSocket(0));
		pkBottle = item2;
	}
