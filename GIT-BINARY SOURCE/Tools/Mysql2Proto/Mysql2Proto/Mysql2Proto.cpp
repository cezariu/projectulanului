...
#ifdef _DEBUG
#pragma comment(lib, "liblz4_static_d.lib")
#else
#pragma comment(lib, "liblz4_static_r.lib")
#endif
...
static bool BuildMobTable(const char * lcName)
{
	...
	std::string localeName = "locale_name_";
	localeName += lcName;
	char query[4096];
...
static void SaveMobProto(const char * lcName)
{
	if (!s_pMobTable)
		return;

	char fileName[256];
	snprintf(fileName, sizeof(fileName), "data/monsters/protos/%s", lcName);
...
static bool BuildItemTable(const char * lcName)
{
...
#endif
	{
		printf("LoadItemProto: invalid item_proto structSize[%u] != sizeof(SItemTable) %zu\n", structSize, sizeof(CItemData::TItemTable));
		return false;
	}
...
static void SaveItemProto(const char * lcName)
{
	if (!s_pItemTable)
		return;

	char fileName[256];
	snprintf(fileName, sizeof(fileName), "data/items/protos/%s", lcName);
...
int main(int argc, char ** argv)
{
	...
	printf("sizeof(TMobTable_r275) %d\n", sizeof(CPythonNonPlayer::TMobTable_r275));
#endif
#endif
	...
	if (bAItemProto)
	{
		if (bXPhase)
			...
			for (int i = 0; i < 10; i++) {
				if (BuildItemTable(languages[i].c_str()))
					SaveItemProto(languages[i].c_str());
			}
		}
	}
	...
	if (bAMobProto)
	{
		if (bXPhase)
			...
			for (int i = 0; i < 10; i++) {
				if (BuildMobTable(languages[i].c_str()))
					SaveMobProto(languages[i].c_str());
			}
		}
	}
