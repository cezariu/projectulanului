...
struct FileCompressor
{
	...
	bool operator()(const std::string& source)
	{
		...
		if (input)
		{
			...
			if ((compressed = LZ4_compress_HC(reinterpret_cast<char*>(&src[0]), reinterpret_cast<char*>(&data[0]), decompressed, LZ4_compressBound(decompressed), LZ4HC_CLEVEL_MIN)) >= decompressed) {
...
int main(int argc, char** argv)
{
	...
	if (argc > 5)
	{
		std::cout << "FUSION2 -- 2022" << std::endl << std::endl;
	}
	else
	{
