...
void CScreen::Show(HWND hWnd)
{
	...
	if (g_isBrowserMode)
	{
		RECT rcTop={0, 0, LONG(ms_d3dPresentParameter.BackBufferWidth), LONG(g_rcBrowser.top)};
		RECT rcBottom={0, LONG(g_rcBrowser.bottom), LONG(ms_d3dPresentParameter.BackBufferWidth), LONG(ms_d3dPresentParameter.BackBufferHeight)};
		RECT rcLeft={0, LONG(g_rcBrowser.top), LONG(g_rcBrowser.left), LONG(g_rcBrowser.bottom)};
		RECT rcRight={LONG(g_rcBrowser.right), LONG(g_rcBrowser.top), LONG(ms_d3dPresentParameter.BackBufferWidth), LONG(g_rcBrowser.bottom)};
