...
void CGraphicTextInstance::Update()
{
	...
	while (begin < end)
	{
		...
		Ymir_MultiByteToWideChar(m_codepage == 0 ? dataCodePage : m_codepage, 0, begin, token - begin, wText, wTextMax);
#else
		Ymir_MultiByteToWideChar(dataCodePage, 0, begin, token - begin, wText, wTextMax);
#endif
		;
					...
					else
					{
						...
						else if (ret == TEXT_TAG_EMOJI_END)
						{
							...
							if (CResourceManager::Instance().IsFileExist(szPath, __FUNCTION__))
							{
								...
								std::string stringPath = szPath;
								boost::replace_all(stringPath, "icon/moji/icon\\", "icon\\");
								boost::replace_all(stringPath, "icon/emoji/icon\\", "icon\\");
								boost::replace_all(stringPath, ".tga.tga", ".tga");
								if (CResourceManager::Instance().IsFileExist(stringPath.c_str(), __FUNCTION__)) {
									CGraphicImage * pImage = (CGraphicImage *)CResourceManager::Instance().GetResourcePointer(stringPath.c_str());
									kEmoji.pInstance = CGraphicImageInstance::New();
									kEmoji.pInstance->SetImagePointer(pImage);
									m_emojiVector.push_back(kEmoji);
									memset(&kEmoji, 0, sizeof(SEmoji));
