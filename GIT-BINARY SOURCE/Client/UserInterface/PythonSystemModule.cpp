...
PyObject * systemSetMetinSize(PyObject * poSelf, PyObject * poArgs) {
	float fValue;
	if (!PyTuple_GetFloat(poArgs, 0, &fValue)) {
		return Py_BuildException();
	}

	CPythonSystem::Instance().SetMetinSize(fValue);
	return Py_BuildNone();
}

PyObject * systemGetMetinSize(PyObject * poSelf, PyObject * poArgs) {
	return Py_BuildValue("f", CPythonSystem::Instance().GetMetinSize());
}
...
void initsystemSetting()
{
	...
	{
		{"SetMetinSize", systemSetMetinSize, METH_VARARGS},
		{"GetMetinSize", systemGetMetinSize, METH_VARARGS},
