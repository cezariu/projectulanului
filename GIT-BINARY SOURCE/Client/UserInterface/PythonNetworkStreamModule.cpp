...
PyObject* netSendGuildSymbol(PyObject* poSelf, PyObject* poArgs)
{
	...
	if (!rkGuildMarkUploader.ConnectToSendSymbol(kAddress, 0, 0, iGuildID, szFileName, &uiError,0))
	{
...
PyObject* netSendSwitchBotClose(PyObject* poSelf, PyObject* poArgs)
{
	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
	rkNetStream.SendSwitchBotClose();
	return Py_BuildNone();
}

PyObject* netSendSwitchBotOpen(PyObject* poSelf, PyObject* poArgs)
{
	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
	rkNetStream.SendSwitchBotOpen();
	return Py_BuildNone();
}

PyObject* netSendSwitchBotChange(PyObject* poSelf, PyObject* poArgs)
{

	PyObject* attr;
	if (!PyTuple_GetObject(poArgs, 0, &attr))
		return Py_BuildException();

	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
	rkNetStream.SendSwitchBotChange(attr);
	return Py_BuildNone();
}
#endif
...
#ifdef ENABLE_SWITCHBOT_WORLDARD
		{ "SBOpen",			netSendSwitchBotOpen,					METH_VARARGS },
		{ "SBClose",		netSendSwitchBotClose,					METH_VARARGS },
		{ "SBChange",		netSendSwitchBotChange,					METH_VARARGS },
