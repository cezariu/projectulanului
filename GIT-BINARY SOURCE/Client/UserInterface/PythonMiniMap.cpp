...
void CPythonMiniMap::Render(float fScreenX, float fScreenY)
{
	...
	if (m_fScale >= 2.0f)
	{
		...
		// Boss
		STATEMANAGER.SetRenderState(D3DRS_TEXTUREFACTOR, 0xFF0065FF);
		...
		// Stone
		STATEMANAGER.SetRenderState(D3DRS_TEXTUREFACTOR, 0xFFFF5F00);
...
void CPythonMiniMap::__LoadAtlasMarkInfo()
{
	...
	_snprintf(szAtlasMarkInfoFileName, sizeof(szAtlasMarkInfoFileName), "locale/it/map/%s_point.txt", rkMap.GetName().c_str());
	// END_OF_LOCALE
			...
			fabs(kInstancePosition.y - fRealY) < ((float) CTerrainImpl::CELLSCALE) * 3.0f / m_fScale)
		{
			...
			*pdwTextColor = pkInstEach->GetNameColor();
			if (pkInstEach->IsStone()) {
				*pdwTextColor = 0xFFFF5F00;
			}
			else if ((pkInstEach->IsEnemy()) && (pkInstEach->GetRank() >= 4)) {
				*pdwTextColor = 0xFF0065FF;
			}
			else {
				*pdwTextColor = pkInstEach->GetNameColor();
			}
