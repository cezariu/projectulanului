	...
	for (itor = m_CharacterTextTailList.begin(); itor != m_CharacterTextTailList.end(); ++itor)
	{
		...
		if (pTitle)
		{
			...
			if (LocaleService_IsEUROPE()) // 독일어는 명칭이 길어 오른정렬
			{
				...
				{
					pTitle->SetPosition(pTextTail->x - (iNameWidth / 2) - iTitleWidth - 4.0f
					, pTextTail->y, pTextTail->z);
				}
				else
				{
					pTitle->SetPosition(pTextTail->x - (iNameWidth / 2)
					, pTextTail->y, pTextTail->z);
				}
			}
			else
			{
				pTitle->SetPosition(pTextTail->x - (iNameWidth / 2) - fxAdd
				, pTextTail->y, pTextTail->z);
			}
			...
			if (pLevel)
			{
				...
				if (LocaleService_IsEUROPE()) // 독일어는 명칭이 길어 오른정렬
				{
					if (GetDefaultCodePage() == CP_ARABIC)
					{
						pLevel->SetPosition(pTextTail->x - (iNameWidth / 2) - iLevelWidth - iTitleWidth - 4.0f
						, pTextTail->y, pTextTail->z);
					}
					else
					{
						pLevel->SetPosition(pTextTail->x - (iNameWidth / 2) - iTitleWidth
						, pTextTail->y, pTextTail->z);
					}
				}
				else
				{
					pLevel->SetPosition(pTextTail->x - (iNameWidth / 2) - fxAdd - iTitleWidth
					, pTextTail->y, pTextTail->z);
				}
				...
				if (pLanguageInstance)
				{
					int iLevelWidth, iLevelHeight;
					pLevel->GetTextSize(&iLevelWidth, &iLevelHeight);
					pLanguageInstance->SetPosition(pTextTail->x - (iNameWidth / 2) - iTitleWidth - iLevelWidth - pLanguageInstance->GetWidth() - 4.0f
					, pTextTail->y - 10.0f);
				}
#endif	
			}
		...
		else
		{
			...
			if (pLevel)
			{
				...
				if (LocaleService_IsEUROPE()) // 독일어는 명칭이 길어 오른정렬
				{
					if (GetDefaultCodePage() == CP_ARABIC)
					{
						pLevel->SetPosition(pTextTail->x - (iNameWidth / 2) - iLevelWidth - 4.0f
						, pTextTail->y, pTextTail->z);
					}
					else
					{
						pLevel->SetPosition(pTextTail->x - (iNameWidth / 2)
						, pTextTail->y, pTextTail->z);
					}
				}
				else
				{
					pLevel->SetPosition(pTextTail->x - (iNameWidth / 2) - fxAdd
					, pTextTail->y, pTextTail->z);
				}
...
#ifdef ENABLE_MULTI_LANGUAGE
				if (pLanguageInstance)
					pLanguageInstance->SetPosition(pTextTail->x - (iNameWidth / 2) - iLevelWidth - pLanguageInstance->GetWidth() - 4.0f
				, pTextTail->y - 10.0f);
#endif
			}
		}
	...
	if (!pCharacterInstance->IsSA()) {
		...
		if (!prLanguage)
		{
			...
			if (pCharacterInstance->IsPC() && bLanguage)
			{
				...
				if (CResourceManager::Instance().IsFileExist(szFileName, __FUNCTION__))
				{
