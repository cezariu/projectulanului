...
void CInstanceBase::SHORSE::Create(const TPixelPosition& c_rkPPos, UINT eRace, UINT eHitEffect)
{
	...
	{
		fScale /= 100.0f;
		rkActor.SetScaleWorld(fScale, fScale, fScale);
	}
...
BOOL CInstanceBase::HasBlockedExp()
{
	return m_bHasBlockedExp ? true : false;
}
#endif
...
)
{
	...
	if (npcRaceIndex != -1)
	{
		...
		SetAcce(nonPlayer.GetNPCWear(c_rkCreateData.m_dwRace, NPC_WEAR_ACCE) - 85000);

		ChangeCostumeEffect(true, nonPlayer.GetNPCWear(c_rkCreateData.m_dwRace, NPC_WEAR_SHINING_ARMOR), true);
		ChangeCostumeEffect(false, nonPlayer.GetNPCWear(c_rkCreateData.m_dwRace, NPC_WEAR_SHINING_WEAPON), true);
...
#ifdef ENABLE_ANTI_EXP_SYSTEM
	SetBlockExpStatus(c_rkCreateData.m_bHasBlockedExp);
#endif
...
//#ifdef ENABLE_SUPPORT_SYSTEM
//	if (IsSupport()) {
//		m_GraphicThingInstance.SetScaleWorld(0.5f, 0.5f, 0.5f);
//	}
//#endif

#if defined(NEW_PET_SYSTEM)
	if ((m_dwRace >= 34041) && (m_dwRace <= 34111)){
		float scale = c_rkCreateData.m_dwLevel * 0.005f + 0.75f;
		m_GraphicThingInstance.SetScaleWorld(scale, scale, scale);
	}
#endif

	if (IsStone()) {
		float s = 0.01f * CPythonSystem::Instance().GetMetinSize() + 1.0f;
		m_GraphicThingInstance.SetScaleWorld(s, s, s);
	}
...
void CInstanceBase::Update()
{
	...
	if (GetRace() >= 34998 && GetRace() <= 34999) {
		if (CPythonSystem::instance().GetHideMode3Status()) {
			if (!IsAffect(AFFECT_INVISIBILITY)) {
				__SetAffect(AFFECT_INVISIBILITY, true);
				m_kAffectFlagContainer.Set(AFFECT_INVISIBILITY, true);
			}
...
#ifdef ENABLE_ANTI_EXP_SYSTEM
void CInstanceBase::SetBlockExpStatus(short bHasBlockedExp)
{
	m_bHasBlockedExp = bHasBlockedExp;
}
#endif
...
void CInstanceBase::SetAcce(DWORD dwAcce, bool wikiPreview)
{
	...
	if (pItemData) {
		...
		D3DXVECTOR3 t;
		if (pItemData->GetItemScale(RaceToJob(race), RaceToSex(race), t)) {
			m_GraphicThingInstance.SetAcceScale(t, RaceToJob(GetRace()));
		}
	}
}
