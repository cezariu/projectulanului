...
void CPythonCharacterManager::SetDogMode(bool status)
{
	...
	for (itor = m_kAliveInstMap.begin(); itor != m_kAliveInstMap.end(); itor++) {
		...
		if (pMobTable && pMobTable->bRank < 4 && pMainInstance && pInstance && pMainInstance != pInstance && pInstance->IsEnemy()) {
			if (status) {
				if (pInstance->GetOriginalRace2() == pInstance->GetRace() && pInstance->GetRace() != 101) {
					pInstance->ChangeRace2(101, 0);
				}
			} else {
				if (pInstance->GetRace() == 101 && pInstance->GetOriginalRace2() != pInstance->GetRace()) {
					pInstance->SetRace(pInstance->GetOriginalRace2());
...
void CPythonCharacterManager::RefreshMetinSize() {
	...
	for (itor = m_kAliveInstMap.begin(); itor != m_kAliveInstMap.end(); itor++) {
		...
		if (pInstance->IsStone()) {
			pInstance->GetGraphicThingInstancePtr()->SetScaleWorld(s, s, s);
		}
	}
}
