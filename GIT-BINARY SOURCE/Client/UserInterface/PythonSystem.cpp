...
float CPythonSystem::GetMetinSize() {
	return m_Config.fMetinSize;
}

void CPythonSystem::SetMetinSize(float fValue) {
	if (fValue <= 0.0f || fValue > 100.0f) {
		return;
	}

	m_Config.fMetinSize = fValue;
	CPythonCharacterManager::Instance().RefreshMetinSize();
}
...
#endif
		else if (!stricmp(command, "METIN_SIZE")) {
			m_Config.fMetinSize = atof(value);
		}
#ifdef ENABLE_PERSPECTIVE_VIEW
