...
{
	std::string stAtlasInfoFileName("data/maps/atlasinfo.txt");
...
{
	m_PropertyManager.Initialize("property.xml");
}
...
void CPythonBackground::Create()
{
	...
	m_SnowEnvironment.Create();
 	if (!RegisterEnvironmentData(1, "d:/ymir work/environment/moonlight04.msenv")) {
		TraceError("background.RegisterEnvironmentData(iIndex = 1)");
	}
 	if (!RegisterEnvironmentData(2, "d:/ymir work/environment/Deloxone/eveningsun/eveningsun_kf.msenv")) {
		TraceError("background.RegisterEnvironmentData(iIndex = 2)");
	}
 	if (!RegisterEnvironmentData(3, "d:/ymir work/environment/Deloxone/overcastday/overcastday_kf.msenv")) {
		TraceError("background.RegisterEnvironmentData(iIndex = 3)");
	}
 	if (!RegisterEnvironmentData(4, "d:/ymir work/environment/Deloxone/rainyday/rainyday_kf.msenv")) {
		TraceError("background.RegisterEnvironmentData(iIndex = 4)");
	}
 	if (!RegisterEnvironmentData(5, "d:/ymir work/environment/Deloxone/sunset/sunset_kf.msenv")) {
		TraceError("background.RegisterEnvironmentData(iIndex = 5)");
	}
 	if (!RegisterEnvironmentData(6, "d:/ymir work/environment/skyboxridackhalloween.msenv")) {
		TraceError("background.RegisterEnvironmentData(iIndex = 6)");
	}
}
...
void CPythonBackground::Warp(DWORD dwX, DWORD dwY)
{
	...
	if (GetEnvironmentData(CPythonSystem::Instance().GetEnvironment(), &c_pEnvironmenData)) {
