...
void CPythonNetworkStream::Discord_Update(int lvl)
{
	...
	discordPresence.state = "Hypnotic2.net";
	discordPresence.details = "Take part in the new adventure! ";
	discordPresence.largeImageKey = "image1";
	discordPresence.largeImageText = "Hypnotic2 - Beyond this world!";
...
bool CPythonNetworkStream::RecvEventManager()
{
	...
	if (subIndex == EVENT_MANAGER_LOAD)
	{
		...
		const int differenceTime = (serverTime - now) == 0 ? (serverTime - now) + summerTime : (serverTime - now) + summerTime;
