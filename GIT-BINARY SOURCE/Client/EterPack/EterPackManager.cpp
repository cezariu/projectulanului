...
void CFileSystem::OutputError(int errorCode, bool showAsError, const std::string & stFilename, const char* func)
{
	std::wstring ws;
	ws.assign(stFilename.begin(), stFilename.end());
	OutputError(errorCode, showAsError, ws, func);
}
void CFileSystem::OutputError (int errorCode, bool showAsError, const std::wstring & stFilename, const char* func)
{
	if (showAsError)
...
bool CFileSystem::GetFromPack (CMappedFile& rMappedFile, const char* c_szFileName, LPCVOID* pData)
{
	...
	if (m_pFoxFS)
	{
		...
		{
			unsigned int dwSize = m_pFoxFS->size(c_szFileName), dwReadSize = 0;
