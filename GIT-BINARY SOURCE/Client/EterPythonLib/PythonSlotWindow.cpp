...
void CSlotWindow::SetCoverButton(DWORD dwIndex, const char * c_szUpImageName, const char * c_szOverImageName, const char * c_szDownImageName, const char * c_szDisableImageName, BOOL bLeftButtonEnable, BOOL bRightButtonEnable)
{
	...
	// NOTE : Cover ©öo¨¡¡ÆAI Plus ©öo¨¡¡ÆA¡í ¡Æ¢®¡¤A©öo¡¤A¨ù¡© AO¨öA AU¥ìa¢¬|..
...
void CSlotWindow::RefreshSlot()
{
	...
	// NOTE : Refresh ¥ìE¢Ò¡× ToolTip ¥ì¥ì ¡Æ¡í¨öA CO¢¥I¢¥U - [levites]
	if (IsRendering())
	{
		TSlot * pSlot;
...
{
	// FIXME : A¨Ï¥ì¥ì¢¯i¢¬| ¥ìa¡¤¢®¡¾e CI¢¥A ¥ì¥ìA©¬¢¯¢® SetTopAI ¥ìC¨úi©öo¢¬¢ç¢¬e Capture¡Æ¢® C¢ç¨úiA¢ç ©öo¢¬¡Æ¢¥U. ¡¾¡¿¡ÆIAC ©ö©¡Ao AU¥ìa.
	//         A¡í ¢¥o ¡¾U¨¬¡íAuAI C¨ª¡ÆaA¡ÍA¡í A¡Ì¨ú¨¡¨ú©¬ CO ¥ìi - [levites]
...
void CSlotWindow::OnRender()
{
	...
	for (itor = m_SlotList.begin(); itor != m_SlotList.end(); ++itor)
	{
		...
		if (rSlot.fCoolTime != 0.0f)
		{
			...
			{
				// A©£A¢¬AOAI ©ø¢®©ø¡©Ao 1AE AI©ø¡í¢Òo¢¬e..
		...
		if (rSlot.bActive) {
			...
			for (uint8_t i = 0; i < 3; i++) {
				if (!rSlot.pHilightEffect[i] || !rSlot.pHilightEffect[i]->IsShow()) {
					continue;
				}

				rSlot.pHilightEffect[i]->SetPosition(ix, iy);
...
BOOL CSlotWindow::GetPickedSlotPointer(TSlot ** ppSlot)
{
	...
	// NOTE : ¢¯¨­AE ¢¬CA¡× ¡ío¢¥U CN¡Æ¡ÀAI ¡¾aA¨ª AI¢Òo¢¥A A¢®A¡í AI¢¯eC¨ª ¢¯¨­AE A¡×¢¯¢®¨ù¡©¨¬IAI ¢¯A¢¬¡ÍAE ¨ú¨¡¡¤¢®¡¤I
	//        A¡À¡¤E¡¤I ¡ÆE¡íoC¨ª ¡Æ¡§A¢¬¡¤I¨öa ¥ì¢´Co AO¢¥A SlotA¨¬ AU¥ì¢¯ ©ö¡ì¨öA ¥ìE¢¥U¢¥A ¨¡?¨ù¨¬A¡í AI¢¯eCN¢¥U. - [levites]
	for (TSlotListIterator itor = m_SlotList.begin(); itor != m_SlotList.end(); ++itor)
	{
		TSlot & rSlot = *itor;
		...
		// NOTE : ItemAI Hide ¥ìC¨úiAOA¡í ¡Æ©¡¢¯i¢¬| A¡×CN..
		if (rSlot.isItem)
		{
