...
CGraphicThing * CRaceData::GetLODModelThing()
{
	if (!m_pLODModelThing)
	{
		...
		if (CResourceManager::Instance().IsFileExist(strLODFileName.c_str(), __FUNCTION__))
		{
...
CAttributeData * CRaceData::GetAttributeDataPtr()
{
	...
	if (!CResourceManager::Instance().IsFileExist(m_strAttributeFileName.c_str(), __FUNCTION__))
		return NULL;
