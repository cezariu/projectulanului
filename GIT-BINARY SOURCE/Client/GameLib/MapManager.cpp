...
{
	if (m_EnvironmentDataMap.size() > 0) {
...
void CMapManager::__LoadMapInfoVector()
{
	CMappedFile kFile;
	LPCVOID pData;
	if (!CFileSystem::Instance().Get(kFile, m_stAtlasInfoFileName.c_str(), &pData, __FUNCTION__)) {
		if (!CFileSystem::Instance().Get(kFile, "AtlasInfo.txt", &pData, __FUNCTION__)) {
			return;
		}
	}
