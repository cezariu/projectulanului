...
void __GetRaceResourcePathes(unsigned race, std::vector <std::string> & vec_stPathes)
{
	...
	else if (__IsNPCRace(race))
	{
		if (race >= 30000)
		{
			...
			{
				vec_stPathes.push_back("d:/ymir work/npc_pet/"); //FOLDER PATCH PET SYSTEM DIRECTORY
			}
#endif
...
bool CRaceManager::__LoadRaceMotionList(CRaceData & rkRaceData, const char * pathName, const char * motionListFileName)
{
	...
	if (!CFileSystem::Instance().Get(kMappedFile, motionListFileName, &pvData, __FUNCTION__))
		return false;
...
void CRaceManager::RegisterRaceSrcName(const std::string& c_szName, const std::string& c_szSrcName)
#else
...
#endif
{
	...
	if (dwRaceIndex >= 10841 && dwRaceIndex <= 10864) {
		*ppRaceData = NULL;
		return false;
	}
#endif
...
void CRaceManager::SetRaceHeight(int iVnum, float fHeight, float fRiding)
{
	TRaceHeight s;
	s.fHeight = fHeight;
	s.fRiding = fRiding;
	
	m_kMap_iRaceKey_fRaceAdditionalHeight.insert(std::map<int, TRaceHeight>::value_type(iVnum, s));
}

float CRaceManager::GetRaceHeight(int iVnum)
{
	std::map<int, TRaceHeight>::iterator it = m_kMap_iRaceKey_fRaceAdditionalHeight.find(iVnum);
	if (m_kMap_iRaceKey_fRaceAdditionalHeight.end() == it)
		return 0.0f;
	
	return it->second.fHeight;
}

float CRaceManager::GetRidingRaceHeight(int iVnum)
{
	std::map<int, TRaceHeight>::iterator it = m_kMap_iRaceKey_fRaceAdditionalHeight.find(iVnum);
	if (m_kMap_iRaceKey_fRaceAdditionalHeight.end() == it)
		return 100.0f;
	
	return it->second.fRiding;
}
#endif
