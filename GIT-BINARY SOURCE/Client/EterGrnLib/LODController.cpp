...
{
#ifdef ENABLE_FPS
	m_dwMinFPS = ANIFPS_MIN;
	m_dwMaxFPS = ANIFPS_MAX;
#endif	
}
...
void CGrannyLODController::UpdateLODLevel(float fDistanceFromCenter, float fDistanceFromCamera)
{
	...
	{	
		...
		if (m_fLODDistance > 0.0f)
			m_dwLODAniFPS = (DWORD)((m_dwMaxFPS - m_dwMinFPS) * fLODFactor / m_fLODDistance + m_dwMinFPS);
			
			if(m_dwLODAniFPS <= 0.0f)
				m_dwLODAniFPS = m_dwMinFPS;
...
#endif
#ifdef ENABLE_FPS
	m_pCurrentModelInstance->Update(m_dwLODAniFPS, m_dwMinFPS, m_dwMaxFPS);
#else
