...
#if defined(ENABLE_CLIENT_VERSION_CHECK)
int32_t g_iClientVersion = 0;
#endif
bool		g_bCheckClientVersion = false;
string	g_stClientVersion = "jhdsa";
...
static bool __LoadGeneralConfigFile(const char* configName)
{
	...
	while (fgets(buf, 256, fp))
	{
...
#if defined(ENABLE_CLIENT_VERSION_CHECK)
		TOKEN("define_client_version")
		{
			str_to_number(g_iClientVersion, value_string);
			fprintf(stdout, "define_client_version: %d\n", g_iClientVersion);
			continue;
		}
#endif
		...
		TOKEN("MAX_CHAMPION_LEVEL")
		{
			...
			if (i > 255)
			{
				sys_err("MAX_CHAMPION_LEVEL is bigger than the allowed limit (255), reseted to %d.", g_MaxChampionLevel);
				break;
			}

			if (i <= 0)
			{
				sys_err("MAX_CHAMPION_LEVEL is lower than or equal to 0, reseted to %d.", g_MaxChampionLevel);
				break;
			}

			g_MaxChampionLevel = i;
		}
#endif
	}
	fclose(fp);
...
void LoadMapConfig()
{
	...
	for(auto& line : vecLines)
	{
		...
		if (vecToken[0] == "price")
		{
			map.price	= (DWORD) atoll(vecToken[1].c_str());
			continue;
		}
		...
		if (vecToken[0] == "items")
		{
			for (unsigned int i = 1; i < vecToken.size(); i++)
				map.items.emplace_back((DWORD) atoll(vecToken[i].c_str()));
			continue;
		}
