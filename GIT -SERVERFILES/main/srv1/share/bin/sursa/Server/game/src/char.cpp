...
bool CAN_ENTER_ZONE(const LPCHARACTER& ch, int map_index)
{
	...
	if (ch->GetLevel() < 95 && map_index == 210)
		return false;

	if (ch->GetLevel() < 85 && map_index == 103)
		return false;
...
#if defined(ENABLE_EXTENDED_BATTLE_PASS)
	auto it = m_listExtBattlePass.begin();
	while (it != m_listExtBattlePass.end()) {
		auto this_it = it++;
		m_listExtBattlePass.erase(this_it);
	}
...
#ifdef ENABLE_ANTI_EXP_SYSTEM
		addPacket.bHasBlockedExp = m_bHasBlockedExp;
#endif
...
void CHARACTER::UpdatePacket()
{
...
#ifdef ENABLE_ANTI_EXP_SYSTEM
	pack.bHasBlockedExp = m_bHasBlockedExp;
#endif
#ifdef ENABLE_MULTI_LANGUAGE
	...
	pack2.sAlignment = 0;
#ifdef ENABLE_ANTI_EXP_SYSTEM
	pack2.bHasBlockedExp = m_bHasBlockedExp;
#endif
...
#ifdef ENABLE_ANTI_EXP_SYSTEM
	tab.bHasBlockedExp = m_bHasBlockedExp;
#endif
...
#ifdef ENABLE_ANTI_EXP_SYSTEM
	m_bHasBlockedExp = t->bHasBlockedExp;
#endif
		...
		case POINT_EXP:
			{
				...
				else
				{
					...
					if (HasBlockedExp())
					{
						...
						if (bottle->GetSocket(1) + amount > EXP_BOTTLE_STORAGE_LIMIT)
							bottle->SetSocket(1, EXP_BOTTLE_STORAGE_LIMIT);
						else
							bottle->SetSocket(1, bottle->GetSocket(1) + amount);
#endif
...
bool CHARACTER::IsGuildMaster() const {
	const auto g = GetGuild();
	return (g && GetPlayerID() == g->GetMasterPID());

}
uint64_t CHARACTER::GetGuildToken() const {
	const auto g = GetGuild();
	if (g && IsGuildMaster())
		return g->GetID() ^ GuildTokenHash;
	return 0;
			...
			if (dwFirstInfo == dwCondition && GetExtBattlePassMissionProgress(bBattlePassType, bMissionIndex, dwMissionType) < dwSecondInfo)
			{
				...
				while (it != m_listExtBattlePass.end())
				{
					...
					if (pkMission->dwBattlePassType == bBattlePassType &&
						pkMission->dwMissionIndex == bMissionIndex &&
						pkMission->dwMissionType == dwMissionType &&
				...
				if (!foundMission)
				{
					if (!IsExtBattlePassRegistered(bBattlePassType, bBattlePassID))
						DBManager::instance().Query("INSERT INTO player.battlepass_playerindex SET player_id=%d, player_name='%s', battlepass_type=%d, battlepass_id=%d, start_time=NOW()", GetPlayerID(), GetName(), bBattlePassType, bBattlePassID);
