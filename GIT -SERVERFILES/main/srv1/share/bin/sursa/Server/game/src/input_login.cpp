...
#if defined(ENABLE_LEGENDS_STUFF)
	if (!c_r.players[pinfo->index].dwID)
	{
		sys_err("Invalid login maybe exploiting %s.", c_r.login);
		d->SetPhase(PHASE_CLOSE);
		return;
	}
#endif
...
void CInputLogin::Empire(LPDESC d, const char * c_pData)
{
	...
	{
		sys_err("overflow login %s, empire selected %u", r.login, p->bEmpire);
		d->SetPhase(PHASE_CLOSE);
		return;
	}
	...
	if (r.bEmpire != 0)
	{
		...
		// You already selected the empire, wtf?
...
#ifdef ENABLE_GUILD_TOKEN_AUTH
	if (!p->guild_id || (p->token ^ CHARACTER::GuildTokenHash) != p->guild_id)
	{
		sys_err("MARK_SERVER: GuildSymbolUpload: invalid token. guild_id %u token %u hash %u", p->guild_id, p->token, CHARACTER::GuildTokenHash);
		d->SetPhase(PHASE_CLOSE);
		return 0;
	}
#endif
...
#ifdef ENABLE_GUILD_TOKEN_AUTH
	if (!p->gid || (p->token ^ CHARACTER::GuildTokenHash) != p->gid)
	{
		sys_err("MARK_SERVER: GuildMarkUpload: invalid token. gid %u token %u hash %u", p->gid, p->token, CHARACTER::GuildTokenHash);
		return;
	}
#endif
