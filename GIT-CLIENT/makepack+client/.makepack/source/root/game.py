...
class GameWindow(ui.ScriptWindow):
	...
	if app.CHANNEL_CHANGE_SYSTEM:
		def BINARY_OnChannelPacket(self, channelID):
			...
			svname = net.GetServerInfo().split(",")[0]
			
			if channelID == 99:
				net.SetServerInfo((localeinfo.CH_99_NAME % str(svname)).strip())
			else:
				net.SetServerInfo((localeinfo.TEXT_CHANNEL % (str(svname), channelID)).strip())
