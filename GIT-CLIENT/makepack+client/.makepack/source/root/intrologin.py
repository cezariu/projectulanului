...
class LoginWindow(ui.ScriptWindow):
	...
	def Open(self):
		...
		self.loginFailureMsgDict = {
			...
			"WRONGVER" : localeinfo.LOGIN_FAILURE_UPDATE,
		...
		self.loginFailureFuncDict = {
			...
			"QUIT"		: app.Exit,
			"WRONGVER" : self.RestartByPatch,
			"UNKNOWN" : self.RestartByPatch,
	...
	def OnLoginFailure(self, error):
		...
		try:
			...
			loginFailureMsg = localeinfo.LOGIN_FAILURE_UPDATE  + error
	...
	def __LoadScript(self, fileName):
		...
		try:
			...
			self.popupMSG = GetObject("popupMSG")
		...
		self.youtube.SetEvent(ui.__mem_func__(self.__OpenWebSite), 2)
		...
		self.chBtn = {}
		posesX = [19, 92, 164, 237]
		posesY = [354, 354, 354, 354]
		text = ["CH1", "CH2", "CH3", "CH4"]
		for i in xrange(len(posesX) and len(posesY) and len(text)):
			self.chBtn[i] = ui.Button()
			self.chBtn[i].SetParent(self.saveBoard)
			self.chBtn[i].SetPosition(posesX[i], posesY[i])
			self.chBtn[i].SetUpVisual(IMG_PATH + "channel/ch1.png")
			self.chBtn[i].SetOverVisual(IMG_PATH + "channel/ch1_down.png")
			self.chBtn[i].SetDownVisual(IMG_PATH + "channel/ch1_selected.png")
			self.chBtn[i].SetText(text[i])						
			self.chBtn[i].SetEvent(self.setChannel, (i+1))
			self.chBtn[i].Show()
		self.popupSpace = ui.Button()
		self.popupSpace.SetParent(self.popupMSG)
		self.popupSpace.SetPosition(posesX[i], posesY[i])
		(x, y) = self.popupSpace.GetGlobalPosition()
		self.popupX = 0
		self.popupY = 0
		return 1
	...
	def delLoginInfo(self, arg):
		...
		if arg:
			if os.path.isfile("UserData/acc%d"%accID):
				...
				self.saveTxts[accID-1].SetText("%d. Restart client to save"%accID)
				self.saveTxts[accID-1].SetPackedFontColor(0xFF544D43)
	...
	def __OpenWebSite(self, idToOpen):
		...
		if idToOpen == 0:
			self.wndWeb.Open("https://discord.gg/8MC6zAyYWd")
		elif idToOpen == 1:
			self.wndWeb.Open("https://discord.gg/8MC6zAyYWd")
		elif idToOpen == 2:
			self.wndWeb.Open("https://discord.gg/8MC6zAyYWd")
