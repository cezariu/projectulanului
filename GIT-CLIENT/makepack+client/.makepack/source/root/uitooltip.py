...
class ItemToolTip(ToolTip):
	...
	def AddItemData(self, itemVnum, metinSlot, attrSlot = 0, flags = 0, unbindTime = 0, window_type = player.INVENTORY, slotIndex = -1, lockedattr = -1, gaya = 0, wiki = 0, shop = False):
		...
		## item.ITEM_TYPE_BLEND
			...
			if itemVnum == 71148 or itemVnum == 40004 or itemVnum == 40008 or itemVnum == 40009 or itemVnum == 71136 or itemVnum == 71135 or itemVnum == 71188 or itemVnum == 71234 or itemVnum == 71235 or itemVnum == 70058 or itemVnum == 70059 or itemVnum == 70060:
				self.__AppendAffectInformation()
		...
		### Use ###
			...
			elif app.ENABLE_ANTI_EXP_SYSTEM:
				if constinfo.IS_EXP_BOTTLE_ITEM(itemVnum):
					storedamount = int(metinSlot[1])
					
					self.AppendSpace(5)
					if storedamount > 0:
						self.AppendTextLine(localeinfo.TOOLTIP_STORED_EXP % (localeinfo.CutNumberExperienceString(storedamount)), self.SPECIAL_POSITIVE_COLOR)
					else:
						self.AppendTextLine(localeinfo.TOOLTIP_NO_STORED_EXP)
	...
	def AppendPrice(self, price):
		...
		self.AppendTextLine(localeinfo.ITEM_IS_YANG  % (localeinfo.NumberToMoneyString(price)), self.GetPriceColor(price))
