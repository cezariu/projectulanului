...
class MenuWindow(ui.ScriptWindow):
	...
	def Func(self, arg):
		...
		if arg == 0:
			...
			self.inventory.OpenRankWindow()
		elif arg == 3:
			self.inventory.OpenRewardWindow()
		elif arg == 4:
			self.inventory.ClickRuneButton()
...
class InventoryWindow(ui.ScriptWindow):
	...
	def CloseRankWindow(self):
		if self.interface:
			self.interface.CloseRankWindow()

	def OpenRewardWindow(self):
		if self.interface:
			self.interface.OpenRewardWindow()

	def OpenRebornWindow(self):
		if self.interface:
			self.interface.OpenRebornWindow()
	...
	def OpenSwitchbot(self):
		...
		net.SBOpen()
		# if self.interface:
			# self.interface.OpenSwitchWindow()
	...
	def __LoadWindow(self):
		...
		try:
			...
			#self.equipmentPageIndex = 1
			#self.equipmentTypeName = self.GetChild("equipmenttypename")
			self.equipmentTab = []
		...
		if app.ENABLE_STOLE_REAL:
			if self.bkImg:
				if self.equipmentPageIndex == 0:
					...
					#self.equipmentTypeName.SetText(uiscriptlocale.EQUIPMENT_PAGE_BUTTON_TOOLTIP_2)
					self.wndEquip.Hide()
	...
	def RefreshBagSlotWindow(self):
		...
		for i in xrange(player.INVENTORY_PAGE_SIZE):
			...
			if not wasActivated and app.ENABLE_ANTI_EXP_SYSTEM:
				import chr
				import constinfo
				if constinfo.IS_ANTI_EXP_RING_ITEM(itemVnum):
					if chr.HasBlockedExp(player.GetMainCharacterIndex()):
			...
			if not wasActivated and app.ENABLE_SUPPORT_SYSTEM and constinfo.IsSupportSeal(itemVnum):
				metinSocket = [player.GetItemMetinSocket(slotNumber, j) for j in xrange(player.METIN_SOCKET_MAX_NUM)]
				isActivated = 0 != metinSocket[0]
				if isActivated:
					self.wndItem.ActivateSlot(i)
		...
		if app.__BL_CHEST_DROP_INFO__:
			if app.IsPressed(app.DIK_LCONTROL):
				...
				if item.HasDropInfo(ItemVNum, isMain) and self.interface:
					self.interface.OpenChestDropWindow(ItemVNum, isMain, slotIndex, player.INVENTORY)
				return
