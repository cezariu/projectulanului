...
class MessengerWindow(ui.ScriptWindow):
	...
	def OnRefreshList(self):
		...
		if app.ENABLE_MESSENGER_TEAM or app.ENABLE_MESSENGER_HELPER:
			idx = 0
	...
	def OnLogout(self, groupIndex, key, name=None):
		if (app.ENABLE_MESSENGER_TEAM and groupIndex == 2) or\
			(app.ENABLE_MESSENGER_HELPER and groupIndex == 3):
			self.OnRemoveList(groupIndex, key)
		else:
			group = self.groupList[groupIndex]
			member = self.__AddList(groupIndex, key, name)
			if not name:
