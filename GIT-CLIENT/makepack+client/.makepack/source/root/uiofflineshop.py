...
class NewOfflineShopBoard(ui.ScriptWindow):
	...
	def __OnAcceptInputPrice(self):
		...
		if self.pageCategory == "create_shop" and self.AddItemSlotIndex != -1:
			if self.IsShopItemsLimited():
				self.__PopupMessage("Item limit reached.")
				self.__OnCancelInputPrice()
				return
	...
	def IsShopItemsLimited(self):
		return (len(self.CreateShopItemsInfos.keys()) + 1) >= offlineshop.OFFLINESHOP_MAX_ITEM_NUM
